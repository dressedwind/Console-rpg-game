...
void display_menu(const vector<string>& options, int selected, const vector<string>art_start) {
    clear(); // Очистка экрана
    for (size_t i = 0; i < options.size(); ++i) {
        if (i == selected) {
			if (options [i] == "Start Game"){
				int start_y = (LINES - start_game_art.size()) / 2;
                int start_x = (COLS - start_game_art[0].size()) / 2;
                for (size_t j = 0; j < start_game_art.size(); ++j) {
                    mvprintw(start_y + j, start_x, start_game_art[j].c_str());
                }
			}else {
